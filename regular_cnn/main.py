...
def draw_extrinsic_curvature(ax, latent_sweep_angles, curvature_profile, color='orange', label='Regular CNN'):
...
def draw_polar_curvature(ax, latent_sweep_angles, curvature_profile, color='orange'):
    base_radius = 2.0
...
def plot_main_analysis(latent_sweep_angles, curvature_profile, learned_coords, ground_truth_angles, learned_angles, vae, save_dir, subplots_dir):
    ...
    save_individual_plot(draw_extrinsic_curvature, "A_Extrinsic_Curvature.png", subplots_dir, 
                         latent_sweep_angles=latent_sweep_angles, curvature_profile=curvature_profile, color='orange', label='Regular CNN')
    ...
    # Dashboard (1x3 Layout)
    fig = plt.figure(figsize=(20, 6))
    
    ax1 = fig.add_subplot(1, 3, 1)
    draw_extrinsic_curvature(ax1, latent_sweep_angles, curvature_profile, color='orange', label='Regular CNN')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, "1_Main_Analysis.png"), dpi=300)
    plt.show()
...
def plot_neuroscience_dashboard(data, ground_truth_angles, curvature_profile, latent_sweep_angles, save_dir, subplots_dir):
    ...
    save_individual_plot(draw_polar_curvature, "Panel3_Polar_Curvature.png", subplots_dir, projection='polar',
                         latent_sweep_angles=latent_sweep_angles, curvature_profile=curvature_profile, color='orange')

    # Dashboard (1x3 Layout)
    fig = plt.figure(figsize=(20, 6))
    plt.suptitle("CNN 'Orientation Cell' Analysis (Reproducing Fig 6/7)", fontsize=16)
    ...
    draw_polar_curvature(ax3, latent_sweep_angles, curvature_profile, color='orange')

    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, "2_Neuroscience_Dashboard.png"), dpi=300)
    plt.show()
...
def main():
    ...
    # Generate Data 
    generator = NeuralManifoldGenerator(pca_components=DIM_PCA)
    ...
    for epoch in range(EPOCHS):
        for x_batch in train_loader:
            ...
            loss.backward()
            optimizer.step()
        if epoch % 200 == 0: print(f"Epoch {epoch} complete")
    ...
    print(f"Regular Experiment Complete! Saved to: {save_dir}")
